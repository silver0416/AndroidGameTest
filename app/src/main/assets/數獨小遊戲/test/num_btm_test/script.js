    ...
    for (let num = 1; num <= 9; num++) {
        ...
        button.addEventListener("click", function() {
            ...
            if (selectedRow !== -1 && selectedCol !== -1) {
                let selectedCell = sudokuTable.rows[selectedRow].cells[selectedCol];
                if (!selectedCell.classList.contains("selected")) {
                    clearSelection();
                }
                if (userInput[selectedRow][selectedCol] !== "") {
                    userInput[selectedRow][selectedCol] = ""; // 清除之前的使用者輸入
                    selectedCell.querySelector("input").value = ""; // 清空輸入框
                }
